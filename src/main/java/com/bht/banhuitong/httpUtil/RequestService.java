...
public class RequestService {
	...
	public String getRequest(List<BasicNameValuePair> params, String url,
		...
		if (response.getStatusLine().getStatusCode() == OK) {
			...
			//TODO 解析resutlt 处理异常情况（异常返回值类型：@@@ERROR@@@1*您还未登录，请您去登录！                         ）
			if (responseEntity.contains("@@@ERROR@@@")) {
				logger.info(TAG+ ",aaaaaaaaaaaaaaa,response:" + responseEntity);
				responseEntity = responseEntity.substring(responseEntity.indexOf("*") + 1);
				SC.say("aaaaaaaaaaaaaaa"+responseEntity);
			}
			
			return responseEntity;
		} else {
			if (responseEntity != null && responseEntity.length() > 0) {
				logger.info(TAG+ ",error response:" + responseEntity);
	 ...
	 * DELETE请求
	 */
	public String delRequest(List<BasicNameValuePair> nameValuePairs,
			String url, Map<String, String> header) throws Exception {
