...
public class DatabaseModulePortlet extends BasePortlet {
	...
	public void init() {
		...
		dbTableModelMenu.setItems(dbTableModelSearchMenu, dbTableModelPrinterMenu,
				dbTableModelInsertIntoModuleTableMenu, separator, addTableModelMenu, updateTableModelMenu);
		...
		tableMenu.setItems(tableSearchMenu, tablePrinterMenu, tableSearchCondMenu, separator, updateTableMenu,
				delTableMenu);
		...
		updateTableModelMenu.addClickHandler(new ClickHandler() {
			...
			public void onClick(MenuItemClickEvent event) {
				...
				ListGridRecord record = dbTableModelListGrid.getSelectedRecord();
				paramMap.put("TABLE_NAME", record.getAttribute("TABLE_NAME"));
				paramMap.put("COMMENTS", record.getAttribute("COMMENTS"));
				dbModelService.updateTableInfo(paramMap, new AsyncCallback<Integer>() {
					...
					public void onSuccess(Integer result) {
						if (result > 0) {
							dbModelService.queryCurDbUserTablesInfo(baseParamMap,
										...
										public void onSuccess(List<Map<String, String>> result) {
											if (result == null || result.isEmpty()) {
												...
												// SC.say("没有符合条件的数据！");
													...
													dbTableModelListGrid.setData(new ListGridRecord[] {});
													dbTableModelListGrid
															.setData(getRecords(result, dbTableModelFieldItems));
											}
										}
									});
							SC.say("更新成功！");
		...
		updateModuleMenu.addClickHandler(new ClickHandler() {
			...
			public void onClick(MenuItemClickEvent event) {
				Map<String, String> paramMap = new HashMap<String, String>();
				ListGridRecord record = moduleListGrid.getSelectedRecord();
				paramMap.put("SMI_ID", record.getAttribute("SMI_ID"));
				paramMap.put("MODULE_NAME", record.getAttribute("MODULE_NAME"));
				paramMap.put("MUDULE_DESC", record.getAttribute("MUDULE_DESC"));
		...
		updateTableMenu.addClickHandler(new ClickHandler() {
			...
			public void onClick(MenuItemClickEvent event) {
				...
				paramMap.put("TABLE_COMMENTS", record.getAttribute("TABLE_COMMENTS"));
				paramMap.put("REMARK", record.getAttribute("REMARK"));
