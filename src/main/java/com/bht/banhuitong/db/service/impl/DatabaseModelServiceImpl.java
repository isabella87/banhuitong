...
public class DatabaseModelServiceImpl extends BaseDbService {
	 ...
	 */
	private final static String allCurUserTablesInfoSql = " SELECT UT.TABLE_NAME,UT.TABLESPACE_NAME,UT.LAST_ANALYZED,UTC.COMMENTS,UTC.TABLE_TYPE FROM USER_TABLES UT JOIN USER_TAB_COMMENTS UTC ON UTC.TABLE_NAME = UT.TABLE_NAME ORDER BY UT.TABLE_NAME ";
	/**
	...
	public Boolean createTable(String tableName, Map<String, String> tableFieldsMap) {
		...
		if (!tableFieldsMap.isEmpty()) {
			...
			for (String key : tableFieldsMap.keySet()) {
				if (key.equals("PRIMARY_KEY") && tableFieldsMap.get(key) != null
						&& !tableFieldsMap.get(key).isEmpty()) {
	...
	public List<Map<String, String>> querySysModuleInfo(Map<String, String> param) {
		...
		StringBuffer sb = new StringBuffer();
		sb.append(
				" SELECT SMI_ID,MODULE_NAME,MODULE_NO,MUDULE_SHORT_NAME,MUDULE_DESC,CREATOR,CREATE_TIME FROM SYS_MODULE_INFO ");
	...
	public int addSysModuleInfo(Map<String, String> param) {
		...
		sysModuleInsertSb.append(
				" INSERT INTO SYS_MODULE_INFO (SMI_ID,MODULE_NAME,MODULE_NO,MUDULE_SHORT_NAME,MUDULE_DESC,CREATOR,CREATE_TIME) ")
		...
		String[] params = { param.get("MODULE_NAME"), param.get("MUDULE_SHORT_NAME"), param.get("MUDULE_DESC"), "admin",
				param.get("MUDULE_SHORT_NAME") };
	...
	public int addSysTableInfo(Map<String, String> param) {
		...
		String[] params = { param.get("TABLE_NAME"), param.get("TABLE_NAME").split("_")[0], param.get("TABLE_COMMENTS"),
				"admin", param.get("TABLE_NAME") };
	...
	public boolean batchInsertAllTableInfotoModuleTable() {
		...
		shortNamesList.addAll(shortModuleNames);

		// 增加按照某个字段排序功能
		Collections.sort(shortNamesList, new Comparator<String>() {
		...
		for (Map<String, String> mapItem : allTableItems) {
			String[] params = { mapItem.get("TABLE_NAME"), mapItem.get("TABLE_NAME").split("_")[0],
